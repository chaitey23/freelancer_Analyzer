...
export default function StatsSection() {
    ...
    const cardsRef = useRef<HTMLDivElement>(null)
    const counterRefs = useRef<(HTMLSpanElement | null)[]>([])
    const rafRef = useRef<number>(0) // ✅ RAF throttle এর জন্য
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            // ✅ Blob animation GSAP থেকে সরানো হয়েছে — এখন CSS করছে (globals.css)
            // ❌ আগের gsap blob loop এখানে ছিল — delete করা হয়েছে

            // 3D tilt on hover — ✅ RAF দিয়ে throttle করা হয়েছে
            const cardEls = cardsRef.current?.querySelectorAll('.stat-card')
            cardEls?.forEach((card) => {
                const el = card as HTMLElement

                el.addEventListener('mousemove', (e: MouseEvent) => {
                    cancelAnimationFrame(rafRef.current) // ✅ আগের frame cancel
                    rafRef.current = requestAnimationFrame(() => { // ✅ নতুন frame এ run
                        const rect = el.getBoundingClientRect()
                        const x = e.clientX - rect.left
                        const y = e.clientY - rect.top
                        const cx = rect.width / 2
                ...
                el.addEventListener('mouseleave', () => {
                    cancelAnimationFrame(rafRef.current) // ✅ leave এ RAF cancel
                    gsap.to(el, {
                        rotateX: 0,
                        rotateY: 0,
                        duration: 0.6,
        ...
        return () => {
            cancelAnimationFrame(rafRef.current) // ✅ cleanup
            ctx.revert()
        }
    }, [])
    ...
    return (
        <section ref={sectionRef} className="relative w-full py-20 overflow-hidden">
            ...
            {/* ✅ Blob গুলো এখন CSS class দিয়ে animate হচ্ছে — globals.css দেখো */}
            <div className="blob-a pointer-events-none absolute top-10 left-1/4 w-80 h-80 rounded-full opacity-[0.18]"
                style={{ background: 'radial-gradient(circle, #6366f1 0%, transparent 70%)', filter: 'blur(50px)' }} />
            <div className="blob-b pointer-events-none absolute bottom-10 right-1/4 w-96 h-96 rounded-full opacity-[0.10]"
                style={{ background: 'radial-gradient(circle, #8b5cf6 0%, transparent 70%)', filter: 'blur(70px)' }} />
            <div className="blob-c pointer-events-none absolute top-1/2 right-12 w-52 h-52 rounded-full opacity-[0.12]"
                style={{ background: 'radial-gradient(circle, #06b6d4 0%, transparent 70%)', filter: 'blur(35px)' }} />
